...
class HungarianLoss(nn.Module):
    def __init__(self, matcher, num_classes, no_obj_weight=0.1) -> None:
        ...
        self.lambda_iou = 5.0  # Increase IoU loss weight
        self.lambda_l1 = 5.0  # L1 loss weight
        self.lambda_ce = 1.0  # Classification loss weight
    ...
    def forward(self, yhat, y):
        ...
        for batch_idx, (pred_idx, tgt_idx) in enumerate(indices):
            ...
            mask = (target_classes != (self.num_classes - 1)).float()
            loss_ce = F.cross_entropy(
                cls_pred[batch_idx], target_classes, weight=self.empty_weight
            )
            losses["loss_ce"] += self.lambda_ce * loss_ce
            src_boxes = bb_pred[batch_idx][pred_idx]
            target_boxes = bb_gt[batch_idx][tgt_idx]
            l1_loss = F.l1_loss(
                src_boxes * mask.unsqueeze(1), target_boxes * mask.unsqueeze(1)
            )
            losses["loss_bbox"] += self.lambda_l1 * l1_loss
            src_boxes_xyxy = box_cxcywh_to_xyxy(src_boxes)
            target_boxes_xyxy = box_cxcywh_to_xyxy(target_boxes)
            iou = box_iou(src_boxes_xyxy, target_boxes_xyxy).diagonal()
            iou_loss = ((1 - iou) * mask).mean()
            losses["loss_iou"] += self.lambda_iou * iou_loss
